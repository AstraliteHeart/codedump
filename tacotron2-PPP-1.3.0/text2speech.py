...
def alignment_metric(
    alignments, input_lengths=None, output_lengths=None, average_across_batch=False
):
    ...
    values, cur_idxs = torch.max(
        alignments, 1
    )  # get max value in column and location of max value

    cur_idxs = cur_idxs.float()
    prev_indx = torch.cat(
        (cur_idxs[:, 0][:, None], cur_idxs[:, :-1]), dim=1
    )  # shift entire tensor by one.
    ...
    alignments.masked_fill_(
        ~get_mask_from_lengths(output_lengths, max_len=alignments.size(2))[:, None, :],
        0.0,
    )
    att_enc_total = torch.sum(alignments, dim=2)  # [B, enc, dec] -> [B, enc]

    # calc max (with padding ignored)
    att_enc_total.masked_fill_(
        ~get_mask_from_lengths(input_lengths, max_len=att_enc_total.size(1)), 0.0
    )
    ...
    att_enc_total.masked_fill_(
        ~get_mask_from_lengths(input_lengths, max_len=att_enc_total.size(1)), 1.0
    )
    encoder_min_focus = att_enc_total.min(dim=1)[0]  # [B, enc] -> [B]

    # calc average max attention (with padding ignored)
    values.masked_fill_(
        ~get_mask_from_lengths(output_lengths, max_len=values.size(1)), 0.0
    )  # because padding
    ...
    return (
        diagonalitys.cpu(),
        avg_prob.cpu(),
        encoder_max_focus.cpu(),
        encoder_min_focus.cpu(),
        encoder_avg_focus.cpu(),
    )
...
def parse_text_into_segments(texts, split_at_quotes=True, target_segment_length=200):
    ...
    texts = [
        f'"{text.replace(quo,"").strip()}"' if i % 2 else text.replace(quo, "").strip()
        for i, text in enumerate(unidecode(texts).split('"'))
    ]
    ...
    texts = [
        clean_text(text)
        for text in texts
        if len(text.strip().replace('"', "").strip()) or len(clean_text(text))
    ]
    assert len(texts)
    ...
    for i, text in enumerate(texts):
        ...
        if (
            split_at_quotes
            and ('"' in text and quote_mode == False)
            or (not '"' in text and quote_mode == True)
        ):
...
def _get_first_over_thresh(x, threshold):
    """Takes [B, T] and outputs first T over threshold for each B (output.shape = [B])."""
    device = x.device
    x = (
        x.clone().cpu().float()
    )  # GPU implementation of argmax() splits tensor into 32 elem chunks, each chunk is parsed forward then the outputs are collected together... backwards
    x[
        :, -1
    ] = threshold  # set last to threshold just incase the output didn't finish generating.
...
def get_first_over_thresh(x, threshold):
    ...
    x = (
        x.clone().cpu().float()
    )  # using CPU because GPU implementation of argmax() splits tensor into 32 elem chunks, each chunk is parsed forward then the outputs are collected together... backwards
    x[
        :, -1
    ] = threshold  # set last to threshold just incase the output didn't finish generating.
    x[x > threshold] = threshold
...
class T2S:
    def __init__(self, device_id, config_base_path):

        self.device = torch.device(device_id)
        self.config_base_path = config_base_path

        # load T2S config
        config = os.path.join(PATH, "t2s_config.json")
        ...
        assert (
            self.tt_current in self.conf["tacotron"]["models"].keys()
        ), "Tacotron default model not found in config models"
        tacotron_path = os.path.join(
            config_base_path,
            self.conf["tacotron"]["models"][self.tt_current]["modelpath"],
        )  # get first available Tacotron
        (
            self.tacotron,
            self.tt_hparams,
            self.tt_sp_name_lookup,
            self.tt_sp_id_lookup,
        ) = self.load_tacotron2(tacotron_path)

        # load WaveGlow
        self.wg_current = self.conf["waveglow"]["default_model"]
        assert (
            self.wg_current in self.conf["waveglow"]["models"].keys()
        ), "WaveGlow default model not found in config models"
        waveglow_path = os.path.join(
            config_base_path,
            self.conf["waveglow"]["models"][self.wg_current]["modelpath"]
        )  # get first available waveglow
        waveglow_confpath = os.path.join(
            config_base_path,
            self.conf["waveglow"]["models"][self.wg_current]["configpath"]
        )
        (
            self.waveglow,
            self.wg_denoiser,
            self.wg_train_sigma,
            self.wg_sp_id_lookup,
        ) = self.load_waveglow(waveglow_path, waveglow_confpath)

        # load torchMoji
        if self.tt_hparams.torchMoji_linear:  # if Tacotron includes a torchMoji layer
            (
                self.tm_sentence_tokenizer,
                self.tm_torchmoji,
                self.tm_model,
            ) = self.load_torchmoji()
        ...
        if self.conf["tacotron"]["use_speaker_ids_file_override"]:
            speaker_ids_fpath = os.path.join(config_base_path, self.conf["tacotron"]["speaker_ids_file"])
            self.tt_sp_name_lookup = {
                name: self.tt_sp_id_lookup[int(ext_id)]
                for _, name, ext_id in load_filepaths_and_text(speaker_ids_fpath)
            }
    ...
    def load_arpabet_dict(self, dict_path):
        ...
        for line in reversed(
            (open(os.path.join(PATH, dict_path), "r").read()).splitlines()
        ):
    ...
    def load_torchmoji(self):
        ...
        with torch.no_grad():
            ...
            st = SentenceTokenizer(
                vocabulary, maxlen, ignore_sentences_with_only_custom=True
            )
            torchmoji = torchmoji_feature_encoding(PRETRAINED_PATH)
            model = torchmoji_emojis(PRETRAINED_PATH)
        return st, torchmoji, model
    ...
    def update_wg(self, waveglow_name):
        (
            ...
            os.path.join(
                ...
                self.conf["waveglow"]["models"][waveglow_name]["modelpath"],
            ),
            os.path.join(
                self.config_base_path,
                self.conf["waveglow"]["models"][waveglow_name]["configpath"],
            ),
        )
    ...
    def load_tacotron2(self, tacotron_path):
        with torch.cuda.device(self.device):
            ...
            - speaker_lookup
            """
            checkpoint = torch.load(tacotron_path + "_no_tf2")  # load file into memory
            hparams = Munch.fromDict(checkpoint["hparams"])

            checkpoint_hparams = hparams  # checkpoint['hparams'] # get hparams
            ...
            tacotron_speaker_name_lookup = checkpoint[
                "speaker_name_lookup"
            ]  # save speaker name lookup
            tacotron_speaker_id_lookup = checkpoint[
                "speaker_id_lookup"
            ]  # save speaker_id lookup
            return (
                model,
                checkpoint_hparams,
                tacotron_speaker_name_lookup,
    ...
    def update_tt(self, tacotron_name):
        (
            ...
            self.conf["tacotron"]["models"][tacotron_name]["modelpath"]
        )
        self.tt_current = tacotron_name

        if self.conf["tacotron"][
            "use_speaker_ids_file_override"
        ]:  # (optional) override
            self.tt_sp_name_lookup = {
                name: self.tt_sp_id_lookup[int(ext_id)]
                for _, name, ext_id in load_filepaths_and_text(
                    self.conf["tacotron"]["speaker_ids_file"]
                )
            }
    ...
    def get_wg_sp_id_from_tt_sp_names(self, names):
        ...
        wv_model_ids = [
            self.wg_sp_id_lookup[int(speaker_id)] for speaker_id in tt_ext_ids
        ]
        return wv_model_ids
    ...
    def get_closest_names(self, names):
        ...
        validated_names = [
            difflib.get_close_matches(name, possible_names, n=2, cutoff=0.01)[0]
            for name in names
        ]  # change all names in input to the closest valid name
        return validated_names

    def emoji_score(self, text):
        text_batch = [text]
        text_batch = [
    ...
    def infer(
        ...
        with torch.no_grad():
            with torch.cuda.device(self.device):
                with tempfile.TemporaryDirectory() as working_directory, tempfile.TemporaryDirectory() as output_directory:
                    ...
                    if textseg_mode == "no_segmentation":
                        ...
                        texts = parse_text_into_segments(
                            text,
                            split_at_quotes=False,
                            target_segment_length=textseg_len_target,
                        )
                    elif textseg_mode == "segment_by_sentencequote":
                        texts = parse_text_into_segments(
                            text,
                            split_at_quotes=True,
                            target_segment_length=textseg_len_target,
                        )
                    else:
                        raise NotImplementedError(
                            f"textseg_mode of {textseg_mode} is invalid."
                        )
                    del text
                    ...
                    for text_index, text in enumerate(texts):
                        ...
                        if (
                            len(text_batch_in_progress) == simultaneous_texts
                        ...
                        if speaker_mode == "not_interleaved":  # non-interleaved
                            batch_speaker_names = speaker_names * -(
                                -simultaneous_texts // len(speaker_names)
                            )
                            ...
                            speaker_mode == "cycle_next"
                        ):  # use next speaker for each text input
                        ...
                        if (
                            0
                        ):  # (optional) use different speaker list for text inside quotes
                            speaker_ids = [
                                random.choice(speakers).split("|")[2]
                                if ('"' in text)
                                else random.choice(narrators).split("|")[2]
                        ...
                        tacotron_speaker_ids = [
                            self.tt_sp_name_lookup[speaker]
                            for speaker in batch_speaker_names
                        ]
                        tacotron_speaker_ids = (
                            torch.LongTensor(tacotron_speaker_ids)
                            .cuda()
                            .repeat_interleave(batch_size_per_text)
                        )

                        # get speaker_ids (waveglow)
                        waveglow_speaker_ids = self.get_wg_sp_id_from_tt_sp_names(
                            batch_speaker_names
                        )
                        waveglow_speaker_ids = [
                            self.wg_sp_id_lookup[int(speaker_id)]
                            for speaker_id in waveglow_speaker_ids
                        ]
                        ...
                        if style_mode == "mel":
                            ...
                            try:
                                (
                                    ...
                                    _,
                                    _,
                                ) = self.tm_sentence_tokenizer.tokenize_sentences(
                                    text_batch
                                )  # input array [B] e.g: ["Test?","2nd Sentence!"]
                                prob = self.tm_model(tokenized)[0]
                                ...
                                emojis = map(lambda x: EMOJIS[x], emoji_ids)
                                emoji_score = [
                                    emoji.emojize(e, use_aliases=True) for e in emojis
                                ]
                            except:
                                raise Exception(
                            ...
                            style_input = (
                                ...
                                .cuda()
                                .half()
                                .repeat_interleave(batch_size_per_text, dim=0)
                            )
                        ...
                        if style_input.size(0) < (simultaneous_texts * batch_size_per_text):
                            diff = -(
                                -(simultaneous_texts * batch_size_per_text)
                                // style_input.size(0)
                            )
                            style_input = style_input.repeat(diff, 1)[
                                : simultaneous_texts * batch_size_per_text
                            ]
                        ...
                        sequence = torch.zeros(
                            text_lengths.size(0), max_len
                        ).long()  # create large tensor to move each text input into
                        for i in range(
                            text_lengths.size(0)
                        ):  # move each text into padded input tensor
                            sequence[i, : sequence_split[i].size(0)] = sequence_split[i]
                        sequence = (
                            sequence.cuda()
                            .long()
                            .repeat_interleave(batch_size_per_text, dim=0)
                        )  # move to GPU and repeat text
                            ...
                            while np.amin(best_score) < target_score:
                                ...
                                for j in range(
                                    ...
                                    sametext_batch = batch[
                                        start:end
                                    ]  # seperate the full batch into pieces that use the same input text
                                    ...
                                    for k, (
                                        ...
                                        enc_min_focus,
                                        enc_avg_focus,
                                    ) in enumerate(sametext_batch):
                                        # factors that make up score
                                        weighted_score = (
                                            avg_prob.item()
                                        )  # general alignment quality
                                        diagonality_punishment = (
                                            (max(diagonality.item(), 1.20) - 1.20)
                                            * 0.5
                                            * diagonality_weighting
                                        )  # smooth pacing
                                        max_focus_punishment = (
                                            max((enc_max_focus.item() - 40), 0)
                                            * 0.005
                                            * max_focus_weighting
                                        )  # getting stuck on pauses/phones
                                        min_focus_punishment = (
                                            max(0.25 - enc_min_focus.item(), 0)
                                            * min_focus_weighting
                                        )  # skipping single enc outputs
                                        avg_focus_punishment = (
                                            max(2.5 - enc_avg_focus.item(), 0)
                                            * avg_focus_weighting
                                        )  # skipping most enc outputs
                                        weighted_score -= (
                                            diagonality_punishment
                                            + max_focus_punishment
                                            + min_focus_punishment
                        ...
                        (
                            ...
                            [x[0][0].T for x in best_generations],
                            [x[1][0].T for x in best_generations],
                            [x[2][0] for x in best_generations],
                            [x[3][0] for x in best_generations],
                        )
                        # pickup the best attempts from each input

                        # stack best output arrays into tensors for WaveGlow
                        gate_batch_outputs = torch.nn.utils.rnn.pad_sequence(
                            gate_batch_outputs, batch_first=True, padding_value=0.0
                        )

                        # get duration(s)
                        if end_mode == "thresh":
                            max_lengths = (
                                get_first_over_thresh(gate_batch_outputs, gate_threshold)
                                + gate_delay
                            )
                        elif end_mode == "max":
                        ...
                        mel_batch_outputs = torch.nn.utils.rnn.pad_sequence(
                            mel_batch_outputs, batch_first=True, padding_value=-11.6
                        ).transpose(1, 2)[:, :, :max_length]
                        mel_batch_outputs_postnet = torch.nn.utils.rnn.pad_sequence(
                            mel_batch_outputs_postnet, batch_first=True, padding_value=-11.6
                        ).transpose(1, 2)[:, :, :max_length]
                        alignments_batch = torch.nn.utils.rnn.pad_sequence(
                            alignments_batch, batch_first=True, padding_value=0
                        )[:, :max_length, :]

                        # Run WaveGlow
                        audio_batch = self.waveglow.infer(
                            mel_batch_outputs_postnet,
                            speaker_ids=waveglow_speaker_ids,
                            sigma=self.wg_train_sigma * 0.95,
                        )
                        audio_denoised_batch = self.wg_denoiser(
                            audio_batch, strength=0.0001
                        ).squeeze(1)
                    
                        # write audio files and any stats
                        audio_bs = audio_batch.size(0)
                        for j, (audio, audio_denoised) in enumerate(
                            zip(
                                audio_batch.split(1, dim=0),
                                audio_denoised_batch.split(1, dim=0),
                            )
                        ):
                            ...
                            mel_outputs = mel_batch_outputs.split(1, dim=0)[j][
                                :, :, :spec_end
                            ]
                            mel_outputs_postnet = mel_batch_outputs_postnet.split(1, dim=0)[
                                j
                            ][:, :, :spec_end]
                            alignments = alignments_batch.split(1, dim=0)[j][
                                :, :spec_end, : text_lengths[j]
                            ]

                            # save audio
                            filename = (
                                f"{filename_prefix}_{counter//300:04}_{counter:06}.wav"
                            )

                            save_path = os.path.join(PATH, working_directory, filename)

                            # add silence to clips (ignore last clip)
                            if cat_silence_s:
                                cat_silence_samples = int(
                                    cat_silence_s * self.tt_hparams.sampling_rate
                                )
                                audio = torch.nn.functional.pad(
                                    audio, (0, cat_silence_samples)
                                )
                            ...
                            if (counter % 300) == 0 or (
                                ...
                                os.system(f'sox "{files_to_merge}" -b 32 "{fpath}"')

                                assert os.path.exists(
                                    fpath
                                ), f"'{fpath}' failed to generate."
                                del files_to_merge
                                ...
                                if (
                                    (
                                        running_fsize / (1024 ** 3)
                                        > self.conf["output_maxsize_gb"]
                                    )
                                    or (len(fpaths) > 300)
                                    or (last_text and last_item)
                                    ...
                                    out_path = os.path.join(
                                        PATH, output_directory, out_name
                                    )
                                    """
                                    out_file = tempfile.NamedTemporaryFile(delete=False, suffix=output_extension)
                                    out_path = out_file.name
                                    print (f'sox {fpath_str} -C 8 "{out_path}"')
                                    os.system(f'sox {fpath_str} -C 8 "{out_path}"')
                                    if running_fsize >= (
                        ...
                        audio_seconds_generated = round(
                            audio_len.item() / self.tt_hparams.sampling_rate, 3
                        )
